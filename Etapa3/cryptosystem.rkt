...
(define (key n)
  (let* ((e (cadr (get-nth-quadruple n)))
         ...
         (a1 (a1 e f)) (b1 (b1 e f)) (c1 (c1 e f))
         (a2 (a2 e f)) (b2 (b2 e f)) (c2 (c2 e f))
         (a3 (a3 e f)) (b3 (b3 e f)) (c3 (c3 e f)))
    (map (lambda (x) (modulo x 27)) (list a1 b1 c1 a2 b2 c2 a3 b3 c3))))
...
(define (message->codes message)
  (map (lambda (x) (if (eq? x #\ ) 0 (- (char->integer x) 96))) (string->list message)))
...
(define (codes->message codes)
  (list->string (map (lambda (x) (if (eq? x 0 ) #\ (integer->char (+ x 96)))) codes)))
...
(define (extend-key key size)
  (let loop ((curr-key key)
             (curr-size size))
    (if (= curr-size 0) '() (if (null? curr-key) (cons (car key) (loop (cdr key) (sub1 curr-size)))
                           (cons (car curr-key) (loop (cdr curr-key) (sub1 curr-size)))))))
...
(define crypt-codes (lambda (func)
                      (lambda (R K)
                        (map (lambda (r k) (func r k)) R (extend-key K (length R))))))

(define encrypt-codes
  (crypt-codes (lambda (m k) (modulo (+ m k) 27))))

(define decrypt-codes
  (crypt-codes (lambda (c k) (modulo (- c k) 27))))
...
(define crypt-message (lambda (func)
                         (lambda (R K)
                           (let ((C (message->codes R)))
                             (codes->message (func C (extend-key K (length C))))))))

(define encrypt-message
  (crypt-message encrypt-codes))

(define decrypt-message
  (crypt-message decrypt-codes))
