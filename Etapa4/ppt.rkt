...
(define (enqueue t q)
  (stream-append q (stream-cons (multiply T1 t)(stream-cons (multiply T2 t) (stream-cons (multiply T3 t) empty-stream)))))

(define ppt-stream-in-tree-order
  (let loop ((queue (stream-cons '(3 4 5) empty-stream)))
    (stream-cons (stream-first queue) (loop (enqueue (stream-first queue) (stream-rest queue))))))
...
(define (pairs G H)
  (let loop-1 ((i 1) (curr-H H))
    (let loop-2 ((j 1) (curr-G G))
      (if (> j i) (loop-1 (+ 1 i) (stream-rest curr-H))
          (stream-cons (cons (stream-first curr-G) (stream-first curr-H)) (loop-2 (+ 1 j) (stream-rest curr-G)))))))
...
(define (coprime? pair)
  (if (= 1 (gcd (car pair) (cdr pair))) #t #f))

(define gh-pairs-stream
  (stream-filter coprime? (pairs odds (stream-rest odds))))
